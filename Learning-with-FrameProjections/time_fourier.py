...
OUTPUT_FILE = os.path.join(SAVE_DIR, "sleep_combined_fourier.pt")
...
def prepare_data():
    if not os.path.exists(OUTPUT_FILE):
        ...
        def get_fourier(data_tensor):
            x_fft = torch.fft.rfft(data_tensor)
            # Stack into (Amplitude, Phase) channels
            return torch.stack([torch.abs(x_fft), torch.angle(x_fft)], dim=1)

        processed_dataset_object = {
            "train": {"samples": torch.from_numpy(X_train).float(), "labels": torch.from_numpy(y_train).long(), "fourier": get_fourier(torch.from_numpy(X_train).float())},
            "val": {"samples": torch.from_numpy(X_val).float(), "labels": torch.from_numpy(y_val).long(), "fourier": get_fourier(torch.from_numpy(X_val).float())},
            "test": {"samples": torch.from_numpy(X_test).float(), "labels": torch.from_numpy(y_test).long(), "fourier": get_fourier(torch.from_numpy(X_test).float())}
        }
...
class SleepEDF_Pretrain_Dataset(Dataset):
    def __init__(self, pt_file_path=OUTPUT_FILE, split="train"):
        ...
        self.fourier = data_obj["fourier"]
        
    def __len__(self):
        return len(self.data)
        
    def __getitem__(self, idx):
        return self.data[idx].unsqueeze(0), self.fourier[idx]
...
class FourierEncoder(nn.Module):
    def __init__(self, in_channels, in_length, out_channels, kernel_size=3, initial_stride=1):
        ...
        self.abs_block1 = BasicBlock(16, 32, kernel_size, 2, 1, True, False, False, True)
        self.abs_block2 = BasicBlock(32, 64, kernel_size, 2, 1, True, False, False, False)

        # Phase branch
        self.ang_conv = nn.Conv1d(in_channels, 16, kernel_size=kernel_size, stride=initial_stride, padding=kernel_size//2)
        self.ang_block1 = BasicBlock(16, 32, kernel_size, 2, 1, True, False, False, True)
        self.ang_block2 = BasicBlock(32, 64, kernel_size, 2, 1, True, False, False, False)

        # 376 matches rfft length of 1501 -> downsampled twice (stride 2)
        self.fc_abs = nn.Linear(376, 1)
        self.fc_angle = nn.Linear(376, 1)
        
    def forward(self, x):
        x_abs = torch.abs(x).float()
        x_ang = torch.angle(x).float()
        
        a = self.abs_conv(x_abs)
        a = F.relu(a)
        a = self.abs_block1(a)
        a = self.abs_block2(a)
        a = self.fc_abs(a).squeeze(-1) 
        
        p = self.ang_conv(x_ang)
        p = F.relu(p)
        p = self.ang_block1(p)
        p = self.ang_block2(p)
        p = self.fc_angle(p).squeeze(-1)
        
        out = torch.cat([a, p], dim=1)  # (B, 128)
        return out

class IsoAlign(nn.Module):
    def __init__(self, backbone, fourier_encoder, DEVICE):
        super(IsoAlign, self).__init__()
        self.encoder, self.fourier_encoder, self.DEVICE = backbone, fourier_encoder, DEVICE
    ...
    def forward(self, batch_t, batch_f):
        _, feat_t = self.encoder(batch_t)
        feat_f = self.fourier_encoder(batch_f)
        return self.nt_xent_loss(feat_t, feat_f)
...
if __name__ == "__main__":
    ...
    # in_channels=2 expects stacked [Amplitude, Phase] tensors created during prepare_data
    fourier_encoder = FourierEncoder(in_channels=2, in_length=TIME_STEPS, out_channels=LATENT_DIM).to(DEVICE)
    
    model = IsoAlign(time_encoder, fourier_encoder, DEVICE).to(DEVICE)
    ...
    for epoch in range(PRETRAIN_EPOCHS):
        model.train()
        total_loss = 0
        for batch_t, batch_f in pretrain_loader:
            optimizer.zero_grad()
            loss = model(batch_t.to(DEVICE), batch_f.to(DEVICE))
            loss.backward()
            optimizer.step()
        ...
        if avg_loss < best_loss:
            best_loss = avg_loss
            torch.save(model.encoder.state_dict(), os.path.join(CHECKPOINT_DIR, "best_time_encoder_fourier.pth"))