...
def setup_model(args, DEVICE):
    if args.model == 'unet':
        ...
        # FIX: Changed in_channels to use args.n_feature instead of hardcoding 1 or 3
        return ResNet1D(in_channels=args.n_feature, base_filters=32, kernel_size=5, stride=1, groups=1, n_block=3, n_classes=args.out_dim, downsample_gap=2, increasefilter_gap=4, use_do=True).cuda(DEVICE)
    else:
...
class FourierAutoencoder(nn.Module):
    def __init__(self, encoder, input_channels, in_length, latent_dim_each=64):
        ...
        # Make the decoder dynamic! The FFT length for real signals is (in_length // 2) + 1
        fft_length = (in_length // 2) + 1
        
        self.decoder_abs = nn.Sequential(
            nn.Linear(latent_dim_each, 64),
            nn.ReLU(),
            nn.Linear(64, fft_length)
        )
    ...
    def forward(self, x):
        ...
        recon_abs = self.decoder_abs(latent[:, :64])  
