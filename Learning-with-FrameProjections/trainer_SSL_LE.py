...
def setup_model_optm(args, DEVICE, classifier=True):
    ...
    #     backbone = ResNet1D(in_channels=args.n_feature, base_filters=32, kernel_size=5, stride=2, groups=1, n_block=args.block, n_classes=args.n_class, downsample_gap=2, increasefilter_gap=4, output_dim=args.out_dim, backbone=True)
        ...
        # Force in_channels to 1 for the sleep dataset to prevent the 3-channel default
        actual_in_channels = 1 if args.dataset == 'sleep' else args.n_feature
        backbone = ResNet1D(in_channels=actual_in_channels, base_filters=32, kernel_size=5, stride=2, groups=1, n_block=args.block, n_classes=args.n_class, downsample_gap=2, increasefilter_gap=4, output_dim=args.out_dim, backbone=True)
    ...
    if args.framework in ['byol', 'simsiam']:
        ...
        spect_encoder = UNET_2D_simp(input_channels=1, spect_freq=args.spect_freq, spect_time=args.spect_time, output_channels=args.n_feature, kernel_size=5, layer_n=32)
        # Ensure FT_backbone uses the correct in_channels
        actual_in_channels = 1 if args.dataset == 'sleep' else args.n_feature
        FT_backbone = FourierEncoder(in_channels=actual_in_channels, in_length=args.len_sw, out_channels=args.n_feature, kernel_size=3)
